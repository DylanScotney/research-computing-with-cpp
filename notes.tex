...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Source folder}
\KeywordTok{/User/me/build/CMakeCatchTemplate}

\CommentTok{# SuperBuild folder}
\KeywordTok{/User/me/build/CMakeCatchTemplate-SuperBuild}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
      ...
      \DataTypeTok{int} \DataTypeTok{const} \NormalTok{error = MPI_Scatter(}
                \NormalTok{(}\DataTypeTok{void}\NormalTok{*) message.c_str(), N, MPI_CHAR, }\CommentTok{// Only significant at root}
                \NormalTok{buffer, }\DecValTok{256}\NormalTok{, MPI_CHAR, }\CommentTok{// Receiver side}
                \DecValTok{0}\NormalTok{, MPI_COMM_WORLD}
      \NormalTok{);}
      \NormalTok{REQUIRE(error == MPI_SUCCESS);}
...
See also clang tools such as \texttt{clang-tidy}, \texttt{clang-format}.
...
\subsubsection{Only then Language}\label{only-then-language}
...
\section{Appendix - Accelerators}\label{appendix---accelerators}

\subsection{Appendix - Accelerators}\label{appendix---accelerators-1}
